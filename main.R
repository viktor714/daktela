...
write_endpoint<-function(endpoint,token,from=NULL,limit=1000){
  ...
  if(total<1){
    ...
    fwrite(res,paste0("/data/in/tables/",prefix,endpoint[[3]],".csv"),append = FALSE, sep=",", sep2=c("{","|","}"))
      ...
      tryCatch(
        {
          ...
          #If i = 0 then initialize the file else append the csv using fwrite from data.table in order to not waste RAM
          fwrite(res,paste0("/data/in/tables/",prefix,endpoint[[3]],".csv"),append = ifelse(i>0,TRUE,FALSE), sep=",", sep2=c("{","|","}"))
  ...
  logfile_created<-file.exists(paste0("/data/in/tables/",prefix,"log.csv"))
  
  log<-data_frame("date"=Sys.time(),"endpoint"=endpoint[[3]],"exported_records"=total,"extraction_time"=time, "call"=call)
  fwrite(log,paste0("/data/in/tables/",prefix,"log.csv"),append=logfile_created)
...
if(logfile_created) app$writeTableManifest(paste0("/data/in/tables/",prefix,"log.csv"), primaryKey = c("date","endpoint"),destination='in.c-Daktela',incremental = incr_load)
  
    
app$writeTableManifest(paste0("/data/in/tables/",prefix,endpoint[[3]],".csv"), primaryKey = endpoint[[4]][names(endpoint[[4]]) %in% c("primary_key","secondary_key")],destination='in.c-Daktela',incremental = incr_load)
